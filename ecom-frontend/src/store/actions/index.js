   ...
   try {
      ...
      dispatch({
         ...
         payload:
            ...
            "Failed to fetch user's addresses",
      });
   }
};
...
export const createUserCart = (sendCartItems) => async (dispatch, getState) => {
   try {
      ...
      dispatch({
         type: 'IS_ERROR',
         payload:
            error?.response?.data?.message || 'Failed to create user cart',
      });
   }
};
...
export const deleteCategoryDashboardAction =
   (setOpenDeleteModal, categoryId, toast, queryString = '', isAdmin = true) =>
   async (dispatch, getState) => {
      try {
         const endpoint = isAdmin
            ? '/admin/categories/'
...
export const createCategoryDashboardAction =
   (sendData, setOpen, reset, toast, queryString = '') =>
   async (dispatch, getState) => {
      try {
         dispatch({ type: 'CATEGORY_LOADER' });
...
export const updateCategoryDashboardAction =
   (sendData, setOpen, categoryID, reset, toast, queryString = '') =>
   async (dispatch, getState) => {
      try {
         dispatch({ type: 'CATEGORY_LOADER' });
...
export const getAllSellersDashboard = (queryString) => async (dispatch) => {
   try {
      ...
      dispatch({
         type: 'IS_ERROR',
         payload:
            error?.response?.data?.message || 'Failed to fetch sellers data',
      });
   }
};
