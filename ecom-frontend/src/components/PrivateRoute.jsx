...
const PrivateRoute = ({publicPage = false, adminOnly = false}) => {
	...
	if(adminOnly){
		if(isSeller && !isAdmin){
			...
			const sellerAllowed = sellerAllowedRoutes.some(path => location.pathname.startsWith(path)); // and this is used to check if the current path is allowed for sellers, if not then redirect to home page
